...
namespace any_tests // test definitions
{
    ...
    void test_vectors() 
    {
        const std::vector<int>& vec = engine::any_cast<std::vector<int> >(makeVec()); 
        ...
        std::vector<int> vec1 = engine::any_cast<std::vector<int> >(makeVec()); 
    ...
    void test_addressof()
    {
        ...
        engine::any test_val(obj);

        class_with_address_op<int> returned_obj = engine::any_cast<class_with_address_op<int> >(test_val);
        check_equal(&val, returned_obj.get(), "any_cast incorrectly works with type that has operator&(): addresses differ");

        check_true(!!engine::any_cast<class_with_address_op<int> >(&test_val), "any_cast incorrectly works with type that has operator&()");
        //check_equal(engine::unsafe_any_cast<class_with_address_op<int> >(&test_val)->get(), ptr, "unsafe_any_cast incorrectly works with type that has operator&()");
    }
